...
function isNodeFormData(v: unknown): v is {
  getHeaders: (...args: unknown[]) => Record<string, string>
...
axiosClient.interceptors.request.use((config) => {
  ...
  if (
    ...
    && !isBrowserFormData(config.data)
    && !isNodeFormData(config.data)
  ) {
